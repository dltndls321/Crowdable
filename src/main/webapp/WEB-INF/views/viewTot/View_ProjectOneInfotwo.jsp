...
function myfuncblur(){
	...
	if(parseInt(donate)<parseInt(fir)){
		$("#fircol").css("color", "red");
		$("#seccol").css("color", "red");
		$("#thircol").css("color", "red");
	}else if(parseInt(donate)>= parseInt(fir) && parseInt(donate)<parseInt(sec) && parseInt(sec)!=0){
		$("#fircol").css("color", "blue");
		$("#seccol").css("color", "red");
		$("#thircol").css("color", "red");
	}
	else if(parseInt(donate)>= parseInt(sec) && parseInt(donate)<parseInt(thir) && parseInt(thir)!=0){
		$("#fircol").css("color", "blue");
...
<div class="w3-card login_regist_table w3-center" style="margin-top: 2%; margin-bottom: 150px;">
	<form action="/board/donateinsert/${boardinfo.num}.do" id="donateform" method="post">
		<table class="w3-table w3-center">
			...
			<tr style="border-top: 1px;border-bottom: 1px; border-color: #888888;">
				<td colspan="1" style="color: #888888">첫번째 금액</td><td id="fircol" colspan="3" class="w3-left"><c:out value="${giftinfo.giftmoneyone}"></c:out><input type="hidden" id="firstvalue" value="${giftinfo.giftmoneyone}"><input name="membernum" type="hidden" id="membernumherehere" value="${memberinfo.num}"></td>
			</tr>
			...
			<tr style="border-top: 1px;border-bottom: 1px; border-color: #888888;" id="gifttwomoney">
				<td colspan="1" style="color: #888888">두번째 금액</td><td id="seccol" colspan="3" class="w3-left" ><c:out value="${giftinfo.giftmoneytwo}"><input type="hidden" id="secvalue" value="${giftinfo.giftmoneytwo}"></c:out></td>
			</tr>
			</c:if>
			...
			<tr style="border-top: 1px;border-bottom: 1px; border-color: #888888;" id="giftthreemoney">
				<td colspan="1" style="color: #888888">세번째 금액</td><td id="thircol" colspan="3" class="w3-left"><c:out value="${giftinfo.giftmoneythree}"><input type="hidden" id="thirvalue" value="${giftinfo.giftmoneythree}"></c:out></td>
			</tr>
			</c:if>
		</table>
		<div>

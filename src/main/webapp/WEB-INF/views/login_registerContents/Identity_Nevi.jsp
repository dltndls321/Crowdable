<%@ page language="java" contentType="text/html; charset=utf-8"
	...
	$(document).ready(function(){
		$('#identitySetProfile').click(function(e){
			 e.preventDefault();
			 <%session.setAttribute("View_Identity","a");%>
			 location.href="/member/whatviewIdentity.do";
		 });
		$('#identityProject').click(function(e){
			 e.preventDefault();
			 <%session.setAttribute("View_Identity","b");%>
			 location.href="/board/whoboardlist.do";
		 });
		$('#identitySponosr').click(function(e){
			 e.preventDefault();
			 <%session.setAttribute("View_Identity","c");%>
			 location.href="/board/donationlist.do";
		 });
	});
...
<div class="w3-main" style="margin-left:250px">
  ...
  <!-- Top header -->
	...
	<header class="w3-container w3-xlarge w3-card">
		<p class=" w3-left">개인정보</p>
		<p class="w3-center">
			<a class="shadow_animate" id="identitySponosr" style="cursor: pointer; margin: 40px;"
				>내 후원현황</a>
			
			<a class="shadow_animate" id="identityProject" style="cursor: pointer; margin: 40px; "
				>내가 만든 프로젝트</a>
		
			<a class="shadow_animate" id="identitySetProfile" style="cursor: pointer; margin: 40px;"
				>프로필 설정</a>
		</p>
	</header>
</div>